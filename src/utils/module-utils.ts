...
export function addRouteToAppRoutingModule(
  host: Tree,
  options: HasImportOptions & BaseOptions
): Tree {
  ...
  const childrenPath = options.flat
    ...
    : normalize(
        "./" + constants.viewsFolder + "/" + strings.dasherize(options.name)
      );
  const loadChildren =
    "../app/" +
    normalize(
  ...
  nodes.forEach(node => {
    if (node.getText().indexOf("routes") > -1 && node.getChildCount() > 0) {
      ...
      const openParenthsList = findNodes(
        nodeList[0],
 ...
 */
export function findRoutingModuleFromOptions(
  host: Tree,
